...
def add_histogram_to_parser(histogram_parser: argparse.ArgumentParser):
    ...
    histogram_parser.add_argument(
        ...
        default="unset",
        nargs="?",
        help="output the histogram pdf image at the path provided. If no filename "
    ...
    histogram_parser.add_argument(
        "--cumulative", action="store_true", help="make the histogram cumulative"
    )
    histogram_parser.add_argument(
        "--csv",
        metavar="<filename>",
    ...
    # Time it
    histogram_parser.add_argument(
        "--time", action="store_true", help="time the diameter algorithm"
    )
...
def run_histogram(args):
    ...
    histogram_main.compute_pdv(
        args.host, recon_input, args.dup_cost, args.trans_cost, args.loss_cost, args
    )
