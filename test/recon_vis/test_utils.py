...
class TestUtils(unittest.TestCase):
        ...
        for node_tuple in temporal_graph:
            for node_tuple_child in temporal_graph[node_tuple]:
                ...
                if is_child_leaf:
                    continue
                self.assertTrue(
                    ordering_dict[node_tuple] < ordering_dict[node_tuple_child]
                )

    def check_strongly_temporally_consistent(
        self, host_dict: dict, parasite_dict: dict, reconciliation: dict
    ):
        """
        ...
        """
        temporal_graph = utils.build_temporal_graph(
            host_dict, parasite_dict, reconciliation
        )
        ...
        host, parasite, consistency_type = utils.build_trees_with_temporal_order(
            host_dict, parasite_dict, reconciliation
        )
    ...
    def check_weakly_temporally_consistent(
        self, host_dict: dict, parasite_dict: dict, reconciliation
    ):
        """
    ...
    def test_detect_strong_consistency_1(self):
        ...
        self.check_strongly_temporally_consistent(
            host_dict, parasite_dict, reconciliation
        )
    ...
    def test_detect_weak_consistency(self):
        ...
        self.check_weakly_temporally_consistent(
            host_dict, parasite_dict, reconciliation
        )
    ...
    def test_deterministic_temporal_order(self):
        ...
        reconciliation = {
            ...
            ("n2", "m2"): [
                ("S", ("T. minor", "T. bottae"), ("T. wardi", "T. talpoides"))
            ],
            ("n3", "m2"): [
                ("S", ("n8", "T. bottae"), ("G. thomomyus", "T. talpoides"))
            ],
            ...
            ("n7", "O. underwoodi"): [
                ("T", ("G. setzeri", "O. underwoodi"), ("G. panamensis", "O. cavator"))
            ],
            ("n8", "T. bottae"): [
                ("T", ("G. actuosi", "T. bottae"), ("G. ewingi", "G. bursarius"))
            ],
        ...
        for i in range(n_repeat):
            current_temporal_graph = utils.build_temporal_graph(
                host_dict, parasite_dict, reconciliation
            )
        ...
        for tree_size in self.size_range:
            for _ in range(self.num_examples_to_test):
                ...
                for d, t, l in itertools.product(range(1, 3), repeat=3):
                    recon_graph, _, _, best_roots = recongraph_tools.DP(
                        recon_input, d, t, l
                    )
                    for reconciliation, _ in histogram_brute_force.BF_enumerate_MPRs(
