...
def BF_enumerate_partial_MPRs(recongraph, mapping_node):
    ...
    for event_node in recongraph[mapping_node]:
        if event_node_type(event_node) in ["S", "T", "D"]:
            for left_mapping_dict in BF_enumerate_partial_MPRs(
                recongraph, first_child(event_node)
            ):
                for right_mapping_dict in BF_enumerate_partial_MPRs(
                    recongraph, second_child(event_node)
                ):
                    recon_tree = {}
            ...
            for child_mapping_dict in BF_enumerate_partial_MPRs(
                recongraph, first_child(event_node)
            ):
                recon_tree = {}
                recon_tree[mapping_node] = [event_node]
...
def BF_find_histogram(recongraph, roots):
    ...
    hist_dict = {}
    recon_trees = [
        recon_tree for recon_tree, root in BF_enumerate_MPRs(recongraph, roots)
    ]
    for recon_tree_i in range(0, len(recon_trees)):
        for recon_tree_j in range(recon_tree_i + 1):
            recon_tree_A = recon_trees[recon_tree_i]
