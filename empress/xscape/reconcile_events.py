...
def A(parasiteTree, hostTree, tip_mapping, ep, eh):
    ...
    if tipEdge(eh, hostTree):
        ...
        # Loss
        loss1 = lossmerge(
            ep, eh, ehLeftChild, C(parasiteTree, hostTree, tip_mapping, ep, ehLeftChild)
        )
...
def merge(CVlist1, CVlist2, ep, eh, epChild1, ehChild1, epChild2, ehChild2, eventType):
    ...
    for v in CVlist1:
        for w in CVlist2:
            ...
            if keepnewCV:
                ...
                CVevents[nswe] = (
                    CVevents[nswe].union(CVallEvents[vsoln]).union(CVallEvents[wsoln])
                )
                ...
                if intersection:
                    ...
                    if CVseen[key]:
                        CVcommonEvents[key] = CVcommonEvents[key].intersection(
                            CVevents[nswe]
                        )
                    else:
...
def allSwitches(parasiteTree, hostTree, tip_mapping, ep, eh):
    ...
    for switchEdge in hostTree:  # for every possible host edge
        if (
            ...
            output.append(
                (switchEdge, C(parasiteTree, hostTree, tip_mapping, ep, switchEdge))
            )
