...
def plot_costs_on_axis(
    ...
    for cost_vector in cost_vectors:
        ...
        if isinstance(region, Polygon):  # non-degenerate
            ...
            coords = list(region.coords)
            axes.plot(
                coords[0][0], coords[0][1], "o", markersize=4, color=color, label=label
            )
            if verbose:
            ...
            try:
                ...
                for r in region:
                    if isinstance(r, Polygon):  # non-degenerate
                        ...
                        if not legend_entry:
                            axes.add_patch(
                                plt.Polygon(coords, color=color, label=label)
                            )
                            legend_entry = True
                        else:
                            axes.add_patch(
                                plt.Polygon(coords)
                            )  # legend entry has been added previously
                        if verbose:
                            print("  Polygon vertices: ", coords)
                        ...
                        raise Exception(
                            "cost vector (%s) has invalid subregion (%s)"
                            % (str(cost_vector), str(type(r)))
                        )
                    area += r.area
                    if verbose:
                ...
                raise Exception(
                    "cost vector (%s) has invalid region (%s)"
                    % (str(cost_vector), str(type(region)))
                )
...
def plotcosts(
    ...
    if outfile is None:
        ...
        title = outfile
    plot_costs_on_axis(
        ax, CVlist, transfer_min, transfer_max, dup_min, dup_max, title, log, verbose
    )
