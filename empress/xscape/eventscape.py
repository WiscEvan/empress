...
def output(
    ...
    optimalCVlist = restrict(
        CVlist, switchMin, switchMax, lossMin, lossMax, regions=regions
    )
    ...
    for count in range(maxCounts, 0, -1):
        ...
        row.extend(
            [
                displayVersion(event[0], root)
                for event in eventsWithCounts
                if event[1] == count
            ]
        )
        writer.writerow(row)
