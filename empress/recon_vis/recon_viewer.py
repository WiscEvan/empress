...
def _print_inconsistent_text(fig: plot_tools.FigureWrapper, fontsize: float):
    ...
    fig.axis.text(
        0, 0, errormsg, horizontalalignment="center", verticalalignment="center"
    )
...
def _set_offsets(tree: tree.Tree):
    ...
    for node in tree.postorder_list():
        ...
        if y_1 is None or node.layout.node_count == 0:
            ...
            node.layout.offset = abs(y_0 - y_1) / (
                node.layout.node_count + render_settings.COUNT_OFFSET
            )
...
def _draw_host_handle(fig: plot_tools.FigureWrapper, root: tree.Node):
    ...
    fig.line(
        (0, root.layout.y),
        (root.layout.x, root.layout.y),
        render_settings.HOST_EDGE_COLOR,
    )


def _render_host_helper(
    fig: plot_tools.FigureWrapper,
    node: tree.Node,
    show_internal_labels: bool,
    font_size: float,
    ...
    if node.is_leaf():
        ...
        if show_internal_labels:
            color = plot_tools.transparent_color(
                render_settings.HOST_NODE_COLOR, render_settings.INTERNAL_NODE_ALPHA
            )
        ...
        fig.line(
            (node_pos.x, left_y), (left_x, left_y), render_settings.HOST_EDGE_COLOR
        )
        fig.line(
            (node_pos.x, right_y), (right_x, right_y), render_settings.HOST_EDGE_COLOR
        )
        _render_host_helper(
            fig, node.left_node, show_internal_labels, font_size, host_tree
        )
        _render_host_helper(
            fig, node.right_node, show_internal_labels, font_size, host_tree
        )


def _render_parasite(
    fig: plot_tools.FigureWrapper,
    parasite_tree: tree.Tree,
...
def _populate_host_tracks(
    node: tree.Node, recon_obj: recon.Reconciliation, host_lookup: dict
):
    """
    ...
    if not (
        event.event_type is recon.EventType.DUPLICATION
...
def _render_parasite_helper(
    ...
    if node.layout.row == left_node.layout.row:
        ...
        node.layout.y = (
            host_node.layout.y + host_node.layout.h_track * host_node.layout.offset
        )

    # Checks to see if transfer node is inconsistent and if it can be fixed
    if event.event_type is recon.EventType.TRANSFER:
        min_col = host_lookup[
            recon_obj.mapping_of(right_node.name).host
        ].parent_node.layout.col
    ...
    _render_parasite_node(
        fig, node, event, font_size, longest_host_name, show_internal_labels, show_freq
    )
...
def _fix_transfer(
    ...
    if (
        min_col >= node_col
        and min_col < max_col
        and not (_is_sharing_track(node, host_node.name, recon_obj))
    ):
...
def _render_parasite_branches(
    ...
    if event.event_type is recon.EventType.COSPECIATION:
        ...
        _render_transfer_branch(
            node_pos, right_pos, fig, node, host_lookup, recon_obj, right_node
        )
...
def _render_loss_branch(
    node_pos: plot_tools.Position,
...
def _render_cospeciation_branch(
    node: tree.Node,
    host_lookup: dict,
    parasite_lookup: dict,
    recon_obj: recon.Reconciliation,
    ...
    if host_node.left_node.name == left_host_node.name:
        ...
        if host_node.layout.lower_v_track < (host_node.layout.x - node_pos.x) / offset:
            host_node.layout.lower_v_track += (
                host_node.layout.x - node_pos.x
            ) / offset + offset
    else:
        stop_row = host_node.left_node.layout.row
        _connect_child_to_parent(
            node, left_node, host_lookup, recon_obj, fig, stop_row=stop_row
        )
    ...
    if host_node.right_node.name == right_host_node.name:
        ...
        if host_node.layout.upper_v_track < (host_node.layout.x - node_pos.x) / offset:
            host_node.layout.upper_v_track += (
                host_node.layout.x - node_pos.x
            ) / offset + offset
    else:
        stop_row = host_node.right_node.layout.row
        _connect_child_to_parent(
            node, right_node, host_lookup, recon_obj, fig, stop_row=stop_row
        )


def _get_children(
    node: tree.Node, recon_obj: recon.Reconciliation, parasite_lookup: dict
):
    """
...
def _render_transfer_branch(
    ...
    if child_host_node.parent_node.layout.col < node.layout.col:
        ...
        y_midpoint = (
            abs(mid_pos.y + node_pos.y) / 2
        )  # value of midpoint between mid_xy and parent node
        ...
        transfer_edge_color = plot_tools.transparent_color(
            render_settings.PARASITE_EDGE_COLOR, render_settings.TRANSFER_TRANSPARENCY
        )
...
def _connect_child_to_parent(
    ...
    while host_node.layout.row != stop_row and host_node.parent_node:
        ...
        sub_parent_pos = plot_tools.Position(
            parent_node.layout.x - (offset * v_track),
...
def _event_color_shape(event: recon.Event):
    ...
    if event.event_type is recon.EventType.COSPECIATION:
        return (
            render_settings.COSPECIATION_NODE_COLOR,
            render_settings.COSPECIATION_NODE_SHAPE,
        )
    if event.event_type is recon.EventType.DUPLICATION:
        return (
            render_settings.DUPLICATION_NODE_COLOR,
            render_settings.DUPLICATION_NODE_SHAPE,
        )
    if event.event_type is recon.EventType.TRANSFER:
