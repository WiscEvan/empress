...
class FigureWrapper:
    ...
    def line(
        ...
        linestyle: str = DEFAULT_LINESTYLE,
    ):
        """
    ...
    def text(
        ...
        point: tuple,
        string: str,
        col: tuple = render_settings.RED,
        size=FONTSIZE,
        h_a: str = DEFAULT_HORIZONTAL_ALIGNMENT,
    ):
        x, y = point
        self.axis.text(
            x,
            ...
            verticalalignment=DEFAULT_VERTICAL_ALIGNMENT_2,
        )
    ...
    def text_v2(
        ...
        if text is not None:
            ...
            path_patch = PathPatch(
                tp, color=col, linewidth=TEXTWIDTH, zorder=TEXT_Z_ORDER
            )
            if border_col:
                path_patch.set_path_effects(
                    [
                        PathEffects.withStroke(
                            linewidth=BORDER_WIDTH, foreground=border_col
                        )
                    ]
                )
    ...
    def triangle(
        ...
        point: Position,
        col: tuple = render_settings.BLACK,
        markersize: int = TRANSFERSIZE,
        rotation: float = render_settings.UP_ARROW_ROTATION,
    ):
        """
        ...
        self.axis.plot(
            ...
            color=col,
            marker=(3, 0, rotation),
            markersize=TRANSFERSIZE,
            linestyle=DEFAULT_TRIANGLE_LINESTYLE,
        )

    def arrow_segment(
        self, point_1: Position, point_2: Position, col: tuple = render_settings.BLACK
    ):
        """
        ...
        arrow_length_x, arrow_length_y = (point_2.x - point_1.x) / 2, (
            point_2.y - point_1.y
        ) / 2
        plt.arrow(
            point_1.x,
