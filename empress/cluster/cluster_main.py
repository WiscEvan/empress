...
def plot_support_histogram(
    plot_file, hist_def, width, tree_name, d, t, l, max_x=None, max_y=None, title=True
):
    """
...
def support_vis(
    species_tree, gene_tree, gene_root, recon_g, cluster_gs, args, tree_data
):
    """
    Creates Support visualization
    :param species_tree <OrderedDict> - the input species tree
...
def perform_clustering(tree_data, d, t, l, k, args):
    ...
    if args.depth is not None:
        graphs, scores, _ = cluster_util.cluster_graph(
            recon_g, gene_root, score, args.depth, k
        )
    elif args.n_splits is not None:
        graphs, scores, _ = cluster_util.cluster_graph_n(
            recon_g, gene_root, score, args.n_splits, mpr_count, k
        )
    else:
        assert False
    # Visualization
    if args.pdv_vis:
        pdv_vis(
            species_tree, gene_tree, gene_root, recon_g, graphs, args, args.parasite
        )
    if args.support_vis:
        support_vis(
            species_tree, gene_tree, gene_root, recon_g, graphs, args, args.parasite
